...
from gallery.users.forms import LoginForm, RegistrationForm, UpdateAccountForm
...
def account():
    ...
    if form.validate_on_submit():
        ...
        if(not ((not form.picture.data) and form.email.data == current_user.email and form.username.data == current_user.username)):
    ...
    return render_template('account.html', title='Account', imagepfp = image_file, form=form)
